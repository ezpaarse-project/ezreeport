...
const template = createTemplateHelperFrom({
  ...
  body: {
    ...
    dateField: 'datetime',
    filters: [
      { name: 'filter-1', raw: { query_string: { query: '-(host:XXX.XX.XXX.X AND sid:"istex-api-harvester")' } }, isNot: false },
      {
        name: 'filter-2',
        raw: {
          bool: {
            should: [
              { query_string: { query: '-(sid:test*)' } },
            ],
          },
        },
        isNot: false,
        ...
        raw: {
          bool: {
            must_not: {
              bool: {
                must: [
                  { prefix: { referer: { value: 'https://revue-sommaire.istex.fr' } } },
                  { match: { mime: 'JSON' } },
                ],
              },
            },
      ...
      { name: 'filter-4', raw: { range: { status: { lt: 299, gte: 200 } } }, isNot: false },
      {
        name: 'host is not XX.XXX.XX.XX, YY.YYY.YY.YYY, etc.', field: 'host', isNot: true, value: ['XX.XXX.XX.XX', 'YY.YYY.YY.YYY', 'ZZ.Z.Z.ZZ'],
      },
      { name: 'filter-not-2', raw: { match_phrase: { publication_date: { query: 'uuuu' } } }, isNot: true },
      { name: 'filter-not-3', raw: { match_phrase: { owner: { query: 'IP - accès temporaire car pas d acces à Janus et en télétravail' } } }, isNot: true },
      {
      ...
      { name: 'filter-not-7', raw: { match_phrase: { sid: { query: 'ezpaarse' } } }, isNot: true },
      { name: 'filter-not-8', raw: { match_phrase: { rtype: { query: 'OPENURL' } } }, isNot: true },
      {
      ...
      },
      {
        ...
        raw: {
          bool: {
            must: [
              { query_string: { query: 'user-agent:(Elastic-Heartbeat* OR *Yandex*) OR ua:(other OR Googlebot OR DataForSeoBot OR PetalBot OR UptimeRobot OR SemrushBot OR MauiBot OR Adsbot OR LTX71 OR BLEXBot OR bingbot OR Applebot OR Amazonbot) OR sid:(other OR uptimerobot)', time_zone: 'Europe/Paris', analyze_wildcard: true } },
            ],
            filter: [],
            should: [],
            must_not: [],
        ...
        figures: [
          {
            ...
            params: {
              labels: [
                ...
                { text: 'Etablissements', format: { type: 'number' }, aggregation: { type: 'cardinality', field: 'istex-id' } },
                { text: 'Période du', format: { type: 'date' }, aggregation: { type: 'min', field: 'datetime' } },
                { text: 'au', format: { type: 'date' }, aggregation: { type: 'max', field: 'datetime' } },
                { text: 'unique count of owner', format: { type: 'number' }, aggregation: { type: 'cardinality', field: 'owner' } },
              ],
            },
            filters: [],
          },
                    ...
                    filters: {
                      filters: {
                        Autre: {
                          bool: {
                            must: [
                              { query_string: { query: 'ua:("other" OR "PetalBot" OR "UptimeRobot" OR "SemrushBot" OR "MauiBot" OR "Adsbot" OR "LTX71" OR "BLEXBot" OR "bingbot" OR "Applebot") OR sid:("other" OR "uptimerobot" OR "ezs-istex")', time_zone: 'Europe/Paris', analyze_wildcard: true } },
                            ],
                            filter: [],
                            should: [],
                            must_not: [],
                        ...
                        'Non renseigné': {
                          bool: {
                            must: [
                              { query_string: { query: 'ua:none AND sid:none', time_zone: 'Europe/Paris', analyze_wildcard: true } },
                            ],
                            filter: [],
                            should: [],
                            must_not: [],
                        ...
                        'Fouille de texte': {
                          bool: {
                            must: [
                              { query_string: { query: 'ua:("Node.js" OR "Python-urllib" OR "libwww-perl" OR "Wget" OR "curl" OR "Python Requests" OR "Java" OR "Apache-HttpClient") OR sid:("istex-api-harvester" OR "istex-dl" OR "scodex-harvest-corpus" OR "tdm-alignement-pf" OR "tdm-ajoute-id")', time_zone: 'Europe/Paris', analyze_wildcard: true } },
                            ],
                            filter: [],
                            should: [],
                            must_not: [],
        ...
        figures: [
          ...
          },
          {
            ...
            params: {
              ...
              columns: [
                { header: 'Nombre de ressources téléchargées', metric: false, aggregation: { type: 'terms', field: 'istex_bundle_size', size: 10 } },
                { header: 'Value', metric: true, styles: { halign: 'right', valign: 'top' } },
              ],
              total: false,
            },
            filters: [],
        ...
        figures: [
          {
            ...
            params: {
              title: 'accès par ua',
              columns: [
                { header: 'user agent', metric: false, aggregation: { type: 'terms', field: 'ua' } },
                { header: 'Total des accès', metric: true, styles: { halign: 'right', valign: 'top' } },
              ],
              total: false,
            },
            filters: [],
        ...
        figures: [
          ...
          },
          {
            ...
            params: {
              title: 'host = ip',
              columns: [
                { header: 'host = ip', metric: false, aggregation: { type: 'terms', field: 'host' } },
                { header: 'total des accès', metric: true, styles: { halign: 'right', valign: 'top' } },
              ],
              total: true,
            },
        ...
        figures: [
          ...
          },
          {
            ...
            params: {
              ...
              columns: [
                { header: 'titres', metric: false, aggregation: { type: 'terms', field: 'publication_title', missing: 'Non renseigné' } },
                { header: 'count', metric: true, styles: { halign: 'right', valign: 'top' } },
              ],
              total: true,
            },
            filters: [],
